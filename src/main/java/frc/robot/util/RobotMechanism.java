...
public class RobotMechanism {
    ...
    StructArrayPublisher<Pose3d> m_poses = NetworkTableInstance.getDefault()
            .getStructArrayTopic("Robot/Component/Poses", Pose3d.struct).publish();
    StructPublisher<Pose3d> publisher = NetworkTableInstance.getDefault()
            .getStructTopic("RobotMechanism/EndEffectorPose", Pose3d.struct).publish();
    ...
    private static final Transform3d k_ElevatorStage3ToCoralHandlerStage1 = new Transform3d(0.1136, 0.0, 0.6501,
            new Rotation3d());
    private static final Transform3d k_CoralHandlerStage1ToStage2 = new Transform3d(0.0956, 0.0, 0.0089,
            new Rotation3d());
    private static final Transform3d k_LeftAlgaeHandler = new Transform3d(0.0005398, 0.29562, 0.13011,
            new Rotation3d(0.0, 0.0, Math.PI / 2.0));
    private static final Transform3d k_RightAlgaeHandler = new Transform3d(0.0005398, -0.29562, 0.13011,
            new Rotation3d(0.0, 0.0, -Math.PI / 2.0));
    private static final Transform3d k_ClimberClamShell = new Transform3d(-0.30481, 0.01196, 0.25714,
            new Rotation3d(0.0, 0.0, Math.PI));
    private static final Transform3d k_ClimberClam2LeftArm = new Transform3d(0.15222, 0.1125, 0.01433,
            new Rotation3d());
    private static final Transform3d k_ClimberClam2RightArm = new Transform3d(0.15222, -0.1125, 0.01433,
            new Rotation3d());
    ...
    public RobotMechanism(Supplier<Pose2d> poseSupplier) {
        m_mechanism = new Mechanism2d(Units.inchesToMeters(15.0),
                Units.inchesToMeters(5) + Constants.Elevator.MAX_HEIGHT_METERS);
        m_root = m_mechanism.getRoot("Elevator", Units.inchesToMeters(15.0 - 7.5), Units.inchesToMeters(4.0));
        MechanismLigament2d m_elevatorBase = new MechanismLigament2d("Elavator Extension", Units.inchesToMeters(5),
                90.0, 10, new Color8Bit(200, 200, 200));
        m_elevatorExtension = new MechanismLigament2d("Elavator Base", Units.inchesToMeters(4), 90.0, 5,
    ...
    public void update() {
        double elevation = RobotContainer.elevator.getElevationMeters();
       ...
       // double leftAlgaeHandlerAngle = RobotContainer.leftAlgaeHandler.isAlgaeIntakeUp() ? 0.0 : Math.PI / 180.0 * 55.0;
        ...
        Transform3d coralHandlerStage1 = new Transform3d(0, 0, 0,
    ...
    public Pose3d getFieldPositionOfCoralHandler() {
        ...
        Transform3d baseToCoralHandlerTransform = new Transform3d(baseToCoralHandler, robotPose.getRotation());

        robotPose = robotPose.transformBy(baseToCoralHandlerTransform);
